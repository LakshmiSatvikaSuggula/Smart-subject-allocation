...
export default function StudentDashboard() {
    ...
    useEffect(() => {
        const fetchElectives = async () => {
            try {
                const res = await fetch("http://localhost:5000/electives", {
                    headers: { Authorization: `Bearer ${token}` }
                });
                const data = await res.json();
                if (data.ok) setElectives(data.electives);
            } catch (err) {
                console.error(err);
            }
        };
    ...
    const renderActiveComponent = () => {
        switch (activeTab) {
            case "preferences":
                return <PreferenceForm electives={electives} student={student} />;
            case "status":
                return <AllocationStatus student={student} electives={electives} />;
            case "download":
                return <ConfirmationSlip student={student} electives={electives} />;
            default:
                return <PreferenceForm electives={electives} student={student} />; 
        }
    };
